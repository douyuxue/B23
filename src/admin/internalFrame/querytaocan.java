...
import admin.table.model.Table_taocan_model;

public class querytaocan extends JInternalFrame {
	public querytaocan(String title, boolean resizable, boolean closable,
			boolean maximizable) {
		...
		final Table_taocan_model model = new Table_taocan_model(20);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(menunos.getText().length() == 0){
					if(menunames.getText().length() == 0){
						...
						try {
							list = menudao.getMenuBytaocanname(menunames.getText());
						} catch (SQLException e1) {
							// TODO Auto-generated catch block
							e1.printStackTrace();
						}
					...
					if(menunames.getText().length() == 0){
						try {
							list = menudao.getMenutaocanByMno(menunos.getText());
						} catch (SQLException e1) {
							// TODO Auto-generated catch block
							e1.printStackTrace();
						}
