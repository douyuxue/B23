...
import admin.table.model.Table_huncai_model;

public class queryhuncai  extends JInternalFrame {
	public queryhuncai(String title, boolean resizable, boolean closable,
			boolean maximizable) {
		...
		final Table_huncai_model model = new Table_huncai_model(20);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(menunos.getText().length() == 0){
					if(menunames.getText().length() == 0){
						...
						try {
							list = menudao.getMenuByhuncainame(menunames.getText());
						} catch (SQLException e1) {
							// TODO Auto-generated catch block
							e1.printStackTrace();
						}
					...
					if(menunames.getText().length() == 0){
						try {
							list = menudao.getMenuhuncaiByMno(menunos.getText());
						} catch (SQLException e1) {
							// TODO Auto-generated catch block
							e1.printStackTrace();
						}
